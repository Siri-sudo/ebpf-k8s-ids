...
def parse_arguments():
    ...
    parser.add_argument('--ports', type=str, default='-', help='Port range for nmap scan (default: all ports)')
...
def run_nmap_scan(target_ip, ports):
    NMAP_SCAN_COMMAND = ["nmap", "-sS", f"-p{ports}", target_ip]
...
def monitor_logs(container_name, start_time_ns, detection_queue):
    log_pattern = re.compile(r"TCP SCAN DETECTED")  # Adjust the pattern to match Snort's output

    kubectl_logs_command = ["kubectl", "logs", container_name, "--follow"]
        ...
        with subprocess.Popen(kubectl_logs_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True) as process:
            for line in process.stdout:
                if log_pattern.search(line):
                    ...
                    latency_ms = (detection_time_ns - start_time_ns) / 1_000_000
                    logging.info(f"Detection time: {latency_ms:.2f} ms")
                    detection_queue.put(latency_ms)
                    break
    except Exception as e:
...
def main():
    ...
    for i in range(num_scans):
        logging.info(f"Starting nmap scan {i+1}/{num_scans} and monitoring logs...")

        # Start Timer for this scan
        start_time_ns = time.perf_counter_ns()
        logging.info(f"Timer started at {start_time_ns} ns")

        detection_queue = queue.Queue()

        # Start nmap scan in a separate thread
        nmap_thread = threading.Thread(target=run_nmap_scan, args=(target_ip, ports))
        nmap_thread.start()
        ...
        while nmap_thread.is_alive() and monitor_thread.is_alive():
            try:
                ...
                logging.info(f"Port scan detected! Latency: {latency_ms:.2f} milliseconds")
    ...
    logging.info(f"Detection times written to {output_file}")
