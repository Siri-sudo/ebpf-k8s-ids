...
def calc_stats(df):
    df['cpu_mean'] = df['cpu_percent'].rolling(window=5).mean()
    df['cpu_se'] = df['cpu_percent'].rolling(window=5).std() / np.sqrt(5)
    df['mem_mean'] = df['memory_percent'].rolling(window=5).mean()
    df['mem_se'] = df['memory_percent'].rolling(window=5).std() / np.sqrt(5)
    return df
...
plt.legend()
plt.grid(True)
plt.savefig('cpu_consumption.pdf', format='pdf')
...
plt.legend()
plt.grid(True)
plt.savefig('memory_consumption.pdf', format='pdf')
